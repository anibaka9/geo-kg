...
export function Pagination({ page, totalPages, total, pageSize, qs }: PaginationProps) {
  ...
  const btnBase =
    "inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors h-9 px-3 border";
  ...
  return (
    <div class="mt-4 flex flex-col sm:flex-row items-center justify-between gap-3">
      <p class="text-sm text-muted-foreground">
        ...
        из <span class="font-medium text-foreground">{total}</span> записей
      </p>
      <div class="flex items-center gap-1">
        {page > 1 ? (
          <a href={href(page - 1)} class={btnDefault}>
            ← Назад
          </a>
        ) : (
        ...
        {pages.map((p) => {
          ...
          if (p === page) {
            return <span class={btnActive}>{p}</span>;
          }
          return (
            <a href={href(p)} class={btnDefault}>
              {p}
            </a>
          );
        })}
