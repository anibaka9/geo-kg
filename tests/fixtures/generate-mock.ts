...
function company(orgType: string | null, name: string, country?: string): License["company"] {
  return {
    ...
    country: fld(
      country ?? "Кыргызстан",
      (country ? [country] : ["Кыргызстан"]) as License["company"]["country"]["value"],
    ),
...
function makeLicense(
  ...
  return {
    ...
    status: status(
      annulled ? null : "ВЕ",
      annulled,
      annulled ? "328" : undefined,
      annulled ? "23.06.17" : undefined,
    ),
    polygon: coords,
    beneficiaries: fld("", []),
    notes: fld("", ""),
...
licenses.push(
  makeLicense(24, { region: "Таласская область", country: "Казахстан", coords: "none" }),
);
licenses.push(
  makeLicense(25, {
...
licenses.push(
  makeLicense(26, {
    ...
    minerals: [mineralTypes[2]!, mineralTypes[0]!],
    annulled: true,
    area: 500,
  }),
);
...
buildDatabase(licenses, "tests/fixtures/mock-licenses.db");
