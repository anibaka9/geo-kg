...
function bindFilterForm() {
  ...
  form?.addEventListener("submit", (e) => {
    ...
    const newUrl = `/map${newQs ? `?${newQs}` : ""}`;
    history.pushState({}, "", newUrl);
    const geojsonUrl = `/api/features.geojson${newQs ? `?${newQs}` : ""}`;
    (map.getSource("licenses") as any)?.setData(geojsonUrl);
    void refreshFilterPanel(newUrl);
  });
}

async function refreshFilterPanel(url: string) {
  ...
  if (!newAside || !currentAside) return;
...
function initMap() {
  ...
  map.on("load", () => {
    ...
    map.on("mouseleave", "license-point", () => {
      map.getCanvas().style.cursor = "";
      map.setFilter("license-point-hover", ["all", ["==", "$type", "Point"], ["==", "id", ""]]);
    });
    ...
    map.on("click", (e: any) => {
      if (
        map.queryRenderedFeatures(e.point, { layers: ["license-fill", "license-point"] }).length ===
        0
      ) {
        closePanel();
      }
